...
function deriveMistakesFromSoe(soeWords, fallbackWords) {
  ...
  // 若無錄音，直接回退 mock
  if (!recordedBlob) {
    await doMock(); enable([btnNext, btnScore]); return;
  }

  // 只接受 mp3 / wav 送至 eval.php；其他格式（webm/mp4/ogg）回退 mock
  ...
  try {
    // 構造表單並送至 eval.php（會話 Cookie 需帶上）
    const fd = new FormData();
    fd.append('text', p.en);
    fd.append('audio', recordedBlob, `clip.${ext}`);
